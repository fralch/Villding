...
function DateInput({ label, onChange, value, showIcon = false }: Props) {
  ...
  const parseDate = typeof value === 'string' ? parseISO(value as any) : value;
  const formatDate = !parseDate ? '' : format(parseDate, 'dd LLLL, yyyy', { locale: es });
  const capitalizedDate = capitalizeWords(formatDate);
